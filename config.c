...
void new_config(void){
  ///---Global configuration.---///
	...
	conn = NULL;             // Connection to X server.
	ewmh = NULL;        // Ewmh Connection.
	screen = NULL;           // Our current screen.
	winlist = NULL;        // Global list of all client windows.

	curws = 0;                  // Current workspace.
	top_win=0;           // Window always on top.
	/* if this is set to true the inner border and outer borders colors will be swapped */
	is_sloppy = true;              // by default use sloppy focus
	focuswin = NULL;            // Current focus window.
	numlockmask = 0;
	monlist = NULL;        // List of all physical monitor outputs.
	randrbase = 0;                         // Beginning of RANDR extension events.
	sigcode = 0;                           // Signal code. Non-zero if we've been interruped by a signal.
	ev  = NULL;
  return;
}
